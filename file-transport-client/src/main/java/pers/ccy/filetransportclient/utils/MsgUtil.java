...
import pers.ccy.filetransportclient.domain.packet.*;
import pers.ccy.filetransportclient.client.FileTransferProtocol;
...
public class MsgUtil {
    ...
    public static FileTransferProtocol buildDownloadRequest(String fileUrl, String fileName, String targetPath){
        FileUploadPacket packet = new FileUploadPacket();
        packet.setFileName(fileName);
        packet.setFilePath(fileUrl);
        packet.setTargetPath(targetPath);
        FileTransferProtocol fileTransferProtocol = new FileTransferProtocol();
        fileTransferProtocol.setTransType(5);
        fileTransferProtocol.setTransObj(packet);
        return fileTransferProtocol;
    }
}
